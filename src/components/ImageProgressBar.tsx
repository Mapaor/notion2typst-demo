...
export default function ImageProgressBar({ current, total, loading }: ProgressBarProps) {
  const percent = total > 0 ? Math.min(100, Math.round((current / total) * 100)) : 0;
  return (
    <div className="w-full mt-6">
      <div className="flex justify-between mb-1">
        <span className="text-sm font-medium text-green-700">Descarregant imatges...</span>
